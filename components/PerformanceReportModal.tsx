...
export default function PerformanceReportModal({
	...
	const totalTargetScore = targets.reduce(
	...
	const downloadPDF = () => {
		...
		doc.save(
			`${employee.name}_performance_report_${selectedYear}_${selectedMonth}.pdf`
		);
	};
				...
				<div className="space-y-4">
					...
					<div className="flex items-center space-x-2">
						<div className="font-bold">
							User's Target Percentage: {totalTargetScore.toFixed(2)}%
						</div>
					</div>
					<div className="flex items-center space-x-2">
						<div className="font-bold">
							Maximum Admin Mark: {maxAdminMark.toFixed(2)}%
						</div>
					</div>
					<div className="flex items-center space-x-2">
						{isEditingAdminMark ? (
							<>
								...
								<Input
									...
									value={adminMark}
									onChange={(e) =>
										setAdminMark(Math.min(Number(e.target.value), maxAdminMark))
									}
									className="w-24"
								/>
							...
							<>
								<div className="font-bold">
									Admin Mark: {adminMark.toFixed(2)}%
								</div>
